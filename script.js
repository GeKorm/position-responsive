...
window.onload = function () {
  //Register mouseenter event TODO: There's probably a cleaner way to do this using the singleton
  document.body.addEventListener('mouseover', winMouseOver);

  //Example usage:
  var pin = new Pin(document.querySelector('#pinned'));
  pin.options.leftMin = 200;
  pin.options.rightMin = 200;
  pin.options.topMin = 200;
  pin.options.bottomMin = 200;
  pin.start();
  var pint = new Pin(document.querySelector('#another'));
  pint.options.ampX = -2;
  pint.options.ampY = -2;
  pint.options.deadzoneX = 200;
  pint.start();
};
